...
public:
	...
	~tLog(){};
	//和日志，打印输出有关
	TEXPORTS void    ngx_log_init();
	TEXPORTS void    ngx_log_stderr(int err, const char *fmt, ...);
	TEXPORTS void    ngx_log_error_core(int level,  int err, const char* filename, const char* function, unsigned int codeline, const char *fmt, ...);
	TEXPORTS u_char* ngx_log_errno(u_char *buf, u_char *last, int err);
